...
- **示例**：`动漫风格anime style清晰精致线条鲜明对比色彩精致五官现代写实动漫冷色调压抑氛围监狱题材场景`
...
4. **一致性检查**：确保同一人物在不同场景中特征完全一致

### 第三步：10模块结构化生成
1. **模块逐一填充**：按照标准结构填充每个模块的具体内容
2. **内容质量检查**：确保每个模块内容充实、描述具体
3. **逻辑连贯性**：检查各模块间的逻辑关系和场景合理性
4. **风格统一性**：确保所有描述符合统一的动漫风格标准

### 第四步：完整提示词整合
1. **模块整合**：将10个模块的内容整合成流畅的完整提示词
2. **关键词优化**：突出重要的视觉关键词，提高AI理解准确性
3. **长度控制**：平衡详细程度和提示词长度的关系
4. **最终检查**：进行全面的质量检查和风格一致性验证
...
```json
{
  ...
  "场景提示词列表": [
    {
      ...
      "图片提示词结构": {
        "模块1_人物": "主角身份描述",
        "模块2_人物外貌特征": "从特征库严格提取的完整外貌描述",
        "模块3_人物状态": "主角动作、表情、情感状态、姿态描述",
        "模块4_配角": "配角身份和数量描述（如无配角则标注'无配角，独自场景'）",
        "模块5_配角外貌特征": "配角外貌描述（特征库提取或符合设定，如无则标注'无'）",
        "模块6_配角状态": "配角动作和反应描述（如无则标注'无'）",
        "模块7_场景布局": "详细的环境背景、建筑、道具、时间地点描述",
        "模块8_人物关系与空间": "人物间空间位置关系、距离、朝向、视线焦点",
        "模块9_光影效果": "光源方向、强度、色温、阴影分布、氛围营造",
        "模块10_动漫风格": "统一的动漫风格标签和视觉特征"
      },
  ...
  "人物特征一致性检查": {
    "主角外貌一致性": {
      "检查结果": "✅完全一致/❌存在差异",
      "一致性详情": "所有场景中周扬外貌特征100%匹配特征库",
      "差异说明": "如有差异，详细说明差异内容"
    },
    "配角外貌一致性": {
      "检查结果": "✅完全一致/❌存在差异",
      "一致性详情": "重要配角外貌在多场景中保持一致",
      "差异说明": "如有差异，详细说明差异内容"
    },
    "整体一致性评估": "✅/❌ + 详细说明和改进建议"
  },
  "动漫风格统一性验证": {
    "风格标签检查": "所有场景是否包含统一的动漫风格标签",
    "视觉元素一致性": "线条、色彩、绘画技法是否保持一致",
    "整体风格评分": "X/10分",
    "风格建议": "针对风格统一性的优化建议"
  },
  "批次质量总结": {
    "优势分析": ["人物特征一致性好", "场景描述丰富", "空间关系清晰"],
    "待优化点": ["某些场景光影描述可以更详细", "个别配角状态描述待完善"],
    "整体评价": "本批次提示词质量评估和综合建议",
    "推荐使用场景": "适合用于哪些类型的AI绘画平台或风格"
  }
}
```
